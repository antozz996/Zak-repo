...
- **Deep Cost Analysis - Area 1**: aggiunta tabella `event_cost_snapshots` per congelare i numeri economici degli eventi, con endpoint `POST /reports/snapshots` e `GET /reports/margins` per salvare snapshot storici e leggere aggregazioni mensili di ricavi, margine medio, profitto medio per persona e incidenza food cost.
